...
export default function GridButton({isPlayed = false, soundPlay, id, handleSoundChange}){
    return (
        <Wrapper isPlayed={isPlayed} onClick={soundPlay}>
            <label onClick={(e) => e.stopPropagation()} htmlFor={id}>🎵</label>
            <input onClick={(e) => e.stopPropagation()} id={id} type="file" onChange={handleSoundChange} />
        </Wrapper>
    )
};
